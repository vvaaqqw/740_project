...
def get_dates(queue, start_date='2012/01/01', end_date='2023/11/01'):
	...
	crypto=pd.DataFrame({'crypto':['litecoin']})
	df=pd.concat([crypto, features,indicators,periods], axis=1)

	#for raw values
	#all kinds of fees and transaction values are in USD. divide by price USD to obtain litecoin
	url_list=[] #stores generated urls
	feature_list=[] #store feature names
	i=0
	while (i<=15): #this loop generates urls for raw values
		url='https://bitinfocharts.com/comparison/'+df['features'][i]+'-'+'litecoin'+'.html'
		...
		i=i+1
	#for indicators
	#all kinds of fees and transaction values are in USD. drop them or recalculate them after converting the raw values to the litecoin
	i=0
	while (i<=15): #this nested while loop generates url structure for all the indicators. for other currencies change litecoin to CURRENCY_NAME
		j=0
		while (j<=8):
			k=0
			while (k<=4):
				url='https://bitinfocharts.com/comparison/'+df['features'][i]+'-'+'litecoin'+'-'+df['indicators'][j]+df['periods'][k]+'.html'
	...
	path2=dir+'/'+'Merged_Unconverted_litecoin_Data.csv'
	...
	filename=dir+'_'+'litecoin_Data.csv'
...
def get_data(start_date='2016/01/01', end_date='2023/11/01'):
	...
	df=get_data('2012/01/01','2020/01/07')
